...
function readRequestBody(req: http.IncomingMessage): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const decodeBody = (raw: Buffer): string => {
      ...
      try { utf8Decoded = new TextDecoder('utf-8', { fatal: true }).decode(raw); } catch { utf8Decoded = null; }

      if (process.platform === 'win32') {
        let gbDecoded: string | null = null;
        try { gbDecoded = new TextDecoder('gb18030', { fatal: true }).decode(raw); } catch { gbDecoded = null; }
  ...
  if (!upstreamResponse.ok) {
    if (upstreamResponse.status === 404 && targetURLs.length > 1) {
      for (let i = 1; i < targetURLs.length; i += 1) {
        ...
        if (upstreamResponse.ok || upstreamResponse.status !== 404) break;
      }
    }
...
export function configureProxy(config: OpenAICompatUpstreamConfig): void {
  upstreamConfig = {
    ...config,
...
export function getProxyBaseURL(target: OpenAICompatProxyTarget = 'local'): string | null {
  if (!proxyServer || !proxyPort) return null;
  const host = target === 'sandbox' ? SANDBOX_HOST : LOCAL_HOST;
  return `http://${host}:${proxyPort}`;
}

export function getProxyStatus(): OpenAICompatProxyStatus {
  return {
    running: Boolean(proxyServer),
