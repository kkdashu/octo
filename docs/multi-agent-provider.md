...
Octo 当前的运行时已经收敛为 Pi-native 装配链路，不再通过单独的 provider 薄封装再转一层。`profile_key` 的作用只是选择模型线路和上游接口，不再表示 SDK 类型。
...
消息进入
  → Router / GroupRuntimeManager / FeishuGroupAdapter
    ...
    → createPiGroupRuntimeFactory() / createPiGroupSessionHost()
      → createAgentSessionFromServices(pi runtime)
        → builtin tools
        → createWorkspaceToolDefs() + adaptOctoTools()
...
`src/runtime/pi-group-runtime-factory.ts` 会把 profile 映射成 Pi 所需的 API 类型：
...
- `src/db.ts`：统一维护 `chats.session_ref`
...
Pi-native 命名统一使用 Workspace / Chat / profile 语义：
...
- MCP 以 Pi 的 extension 方式注入，不再依赖已删除的 provider 兼容层
- tool name 统一映射为 `mcp__<server>__<tool>`
- 可按 Workspace 技能决定是否启用某些 MCP server
