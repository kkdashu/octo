# Octo 项目文档

## 概述

Octo 是一个多群 AI Agent 编排系统，接入飞书群消息，把每个群映射到独立工作区、独立 Pi session 和独立工具上下文。当前底层运行时已经统一为 `PiProvider`。

## 当前架构

```text
即时通讯平台
  → ChannelManager
    → Router
      → GroupQueue
        → resolveAgentProfile(group.profile_key)
        → PiProvider
          → pi-mono coding-agent
            → builtin tools
            → octo custom tools
            → Pi MCP extensions
```

## 核心模块

### 通道层

- `src/channels/manager.ts`：多通道管理、消息发送、图片发送
- `src/channels/feishu.ts`：飞书接入

### 运行时

- `src/providers/pi.ts`：唯一 provider，负责启动和恢复 Pi session
- `src/providers/pi-session-ref.ts`：本地 session ref 管理
- `src/providers/pi-tool-adapter.ts`：把 Octo tools 适配成 Pi custom tools
- `src/providers/pi-mcp-extension.ts`：把外部 MCP server 适配成 Pi extensions
- `src/providers/prompt-normalizer.ts`：规范化 prompt，并接入图片预处理
- `src/runtime/profile-config.ts`：读取 profile 配置并解析默认 profile
- `src/runtime/external-mcp-config.ts`：读取外部 MCP 配置
- `src/runtime/image-message-preprocessor.ts`：图片消息预处理与缓存
- `src/runtime/minimax-token-plan-mcp.ts`：调用 MiniMax 图片理解能力

### 调度与状态

- `src/group-queue.ts`：每群串行、全局并发限制、session 恢复、group memory 注入
- `src/router.ts`：触发规则与消息聚合
- `src/task-scheduler.ts`：cron 定时任务
- `src/db.ts`：SQLite 存储

### 工具层

- `src/tools.ts`：统一定义业务工具
- `src/runtime/minimax-image.ts`：MiniMax 文生图
- 所有业务工具都通过 Pi custom tools 暴露给 agent

### 管理后台

- `src/admin/api.ts`：后台 API
- `src/admin/App.tsx`：后台前端
- `src/admin/types.ts`：DTO 类型

## 数据模型

Pi-native 重构后，关键字段语义如下：

- `registered_groups.profile_key`：当前群模型线路
- `sessions.session_ref`：Pi 本地 session 文件引用
- `group_memories`：群级长期记忆

与旧架构相比：

- 不再使用 `agent_provider`
- 不再使用 `session_id`
- 切 profile 不会自动删除 session

## 群组目录

```text
groups/
├── main/
│   ├── AGENTS.md
│   └── .pi/
│       ├── skills/
│       └── sessions/
├── feishu_{chatId}/
│   ├── AGENTS.md
│   └── .pi/
│       ├── skills/
│       └── sessions/
├── MAIN_AGENTS.md
└── GROUP_AGENTS.md
```

启动时如果发现旧目录结构，会自动做一次复制迁移：

- `CLAUDE.md` → `AGENTS.md`
- `.claude/skills` → `.pi/skills`

运行时只读取新路径。

## 请求流

```text
飞书消息
  → insertMessage(db)
  → Router 轮询触发
  → GroupQueue.enqueue()
  → 读取 group.profile_key 与 session_ref
  → PiProvider.startSession()
  → normalizePromptForAgent()
  → ImageMessagePreprocessor.preprocess()
  → AgentEvent.text / result / error
  → channelManager.send()
  → saveSessionRef()
```

## Profile 路由

`profile_key` 只决定模型线路和上游接口，底层运行时始终是 Pi。

`src/runtime/profile-config.ts` 负责：

- 加载配置文件
- 回退到 `defaultProfile`
- 解析 API key
- 修正 Moonshot 兼容 endpoint

主群可通过以下工具运维：

- `list_profiles`
- `switch_profile`
- `list_groups`
- `register_group`
- `refresh_groups`

## 群级记忆

- 长期记忆存储在 SQLite `group_memories` 表
- 支持 builtin key：`topic_context`、`response_language`、`response_style`、`interaction_rule`
- 也支持 custom key，但 key 只能使用小写字母和下划线
- 普通群只能管理自己的 memory；主群可以跨群管理
- 记忆只在新 session 启动时注入，避免对 active session 做热更新
- 定时任务通过 `GroupQueue.enqueue()` 启动时，也会复用该群的 memory

## 技能与外部 MCP

- system skills 与 curated skills 都安装到 `.pi/skills/`
- `list_curated_skills` / `install_curated_skill` 用于按群安装技能
- 外部 MCP 由 `config/external-mcp*.json` 描述
- `PiProvider` 启动时把已启用 MCP server 转成 Pi extensions 注入

当前的一个典型 gate 例子是 `pdf-to-markdown`：

- 群目录存在 `.pi/skills/pdf-to-markdown/SKILL.md`
- 对应 `markitdown` MCP server 才会被启用

## 图片处理

- 飞书收到的图片仍按 `media/<chatId>/<messageId>.<ext>` 落盘
- 主模型不直接读原始图片路径，而是先走统一图片理解
- 图片理解结果以纯文本块注入 prompt
- 缓存键为 `sha256(file bytes) + prompt_version`
- 若图片理解失败，会降级成失败说明文本，不阻断整条消息

## 环境变量

- `FEISHU_APP_ID`
- `FEISHU_APP_SECRET`
- `MAIN_GROUP_CHAT_ID`
- `ANTHROPIC_API_KEY`
- `OPENAI_API_KEY`
- `MOONSHOT_API_KEY`
- `MINIMAX_API_KEY`
- `MINIMAX_API_HOST`
- `MINIMAX_MCP_COMMAND`
- `AGENT_PROFILES_PATH`
- `EXTERNAL_MCP_CONFIG_PATH`
- `PORT`
- `LOG_LEVEL`
- `TZ`

## 运维说明

- 切线路用 `switch_profile`
- 重置 AI 上下文用 `clear_session`
- 新群默认 profile 来自 `defaultProfile`
- 排查单群问题时，直接进入对应 `groups/<folder>/` 查看 `AGENTS.md`、`.pi/skills/` 和 `.pi/sessions/`
