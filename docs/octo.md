...
Octo 当前的一等模型是 `Workspace / Chat / Run`。外部入口例如飞书群、CLI、desktop 会先绑定到 Workspace 与 Chat，再进入统一的 Pi runtime。当前底层运行时已经统一为 `PiProvider`。
...
- `src/group-queue.ts`：legacy group 队列与兼容层
