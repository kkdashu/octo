...
function resolveUpstreamAPIType(profile: ResolvedAgentProfile): UpstreamApiType {
  if (profile.upstreamApi) {
    return profile.upstreamApi;
  }

  return profile.provider?.toLowerCase() === "openai" ? "responses" : "chat_completions";
}
...
function normalizeResponsesToolsFromChat(toolsInput: unknown): Array<Record<string, unknown>> {
  ...
  for (const tool of toArray(toolsInput)) {
    ...
    const normalized: Record<string, unknown> = { type: 'function', name };
...
function normalizeResponsesToolChoiceFromChat(toolChoice: unknown): unknown {
  ...
  if (!toolChoiceObj) return toolChoice;

  const normalizedType = toString(toolChoiceObj.type).toLowerCase();
  if (normalizedType === 'any') return 'required';
  if (normalizedType === 'auto' || normalizedType === 'none' || normalizedType === 'required') return normalizedType;
...
function convertChatCompletionsRequestToResponsesRequest(
  ...
  if (chatRequest.model !== undefined) request.model = chatRequest.model;
  if (chatRequest.stream !== undefined) request.stream = chatRequest.stream;
  if (chatRequest.temperature !== undefined) request.temperature = chatRequest.temperature;
  if (chatRequest.top_p !== undefined) request.top_p = chatRequest.top_p;
  ...
  for (const message of toArray(chatRequest.messages)) {
    ...
    if (role === 'tool') {
      ...
      if (toolCallId && output) {
        input.push({ type: 'function_call_output', call_id: toolCallId, output });
      }
      continue;
    }
    ...
    if (role === 'assistant') {
      ...
      for (const toolCall of toArray(messageObj.tool_calls)) {
        ...
        const extraContent = normalizeToolCallExtraContent(toolCallObj);
        if (extraContent !== undefined) functionCallItem.extra_content = extraContent;
        input.push(functionCallItem);
        unresolvedFunctionCalls.set(callId, { name, hasOutput: false });
      }
      continue;
    }
    ...
    if (userParts.length > 0) {
      input.push({ role: role || 'user', content: userParts });
    }
  }
  ...
  for (const messageItem of input) {
    if (toString(messageItem.type) !== 'function_call_output') continue;
  ...
  for (const [callId, callInfo] of unresolvedFunctionCalls.entries()) {
    if (callInfo.hasOutput) continue;
    input.push({
...
function remapMessageRolesForMiniMax(
  ...
  for (const message of messages) {
    ...
    if (!messageObj) continue;
    const role = toString(messageObj.role);
    if (role === 'developer') messageObj.role = 'system';
  }
}
...
function extractMaxTokensRange(errorMessage: string): { min: number; max: number } | null {
  ...
  if (!normalized.includes('max_tokens')) return null;
...
function clampMaxTokensFromError(
  ...
  if (nextValue === currentMaxTokens) return { changed: false };
...
function normalizeMaxTokensFieldForOpenAIProvider(
  ...
  if (provider !== 'openai') return;
  if (!shouldUseMaxCompletionTokensForModel(openAIRequest.model)) return;
...
function mergeSystemMessagesForProvider(openAIRequest: Record<string, unknown>): void {
  ...
  if (systemTexts.length <= 1) return;
...
function isToolsUnsupportedError(errorMessage: string): boolean {
  const normalized = errorMessage.toLowerCase();
  return (
    normalized.includes('does not support tools') || normalized.includes('tool use is not supported')
  );
}
...
function convertMaxTokensToMaxCompletionTokens(
  ...
  const maxTokens = openAIRequest.max_tokens;
  if (typeof maxTokens !== 'number' || !Number.isFinite(maxTokens)) return { changed: false };
  openAIRequest.max_completion_tokens = maxTokens;
  delete openAIRequest.max_tokens;
  ...
  return new Promise((resolve, reject) => {
    ...
    const decodeBody = (raw: Buffer): string => {
      ...
      const collectStringValues = (input: unknown, out: string[]): void => {
        if (typeof input === 'string') { out.push(input); return; }
        if (Array.isArray(input)) { for (const item of input) collectStringValues(item, out); return; }
      ...
      const scoreDecodedJsonText = (text: string): number => {
        let parsed: unknown;
        try { parsed = JSON.parse(text); } catch { return -10000; }
      ...
      if (raw.length >= 3 && raw[0] === 0xef && raw[1] === 0xbb && raw[2] === 0xbf) {
        return createTextDecoder("utf-8", false).decode(raw.subarray(3));
      }
      if (raw.length >= 2 && raw[0] === 0xff && raw[1] === 0xfe) {
        return createTextDecoder("utf-16le", false).decode(raw.subarray(2));
      }
      if (raw.length >= 2 && raw[0] === 0xfe && raw[1] === 0xff) {
        return createTextDecoder("utf-16be", false).decode(raw.subarray(2));
      }

      let utf8Decoded: string | null = null;
      try { utf8Decoded = createTextDecoder("utf-8", true).decode(raw); } catch { utf8Decoded = null; }

      if (process.platform === 'win32') {
        let gbDecoded: string | null = null;
        try { gbDecoded = createTextDecoder("gb18030", true).decode(raw); } catch { gbDecoded = null; }

        if (utf8Decoded && gbDecoded) {
          const utf8Score = scoreDecodedJsonText(utf8Decoded);
          const gbScore = scoreDecodedJsonText(gbDecoded);
          if (gbScore > utf8Score) return gbDecoded;
          return utf8Decoded;
        }

        if (gbDecoded && !utf8Decoded) return gbDecoded;
      }

      if (utf8Decoded) return utf8Decoded;
...
function extractResponsesReasoningText(itemObj: Record<string, unknown>): string {
  ...
  if (summaryTexts.length > 0) return summaryTexts.join('');
...
function convertResponsesToOpenAIResponse(body: unknown): Record<string, unknown> {
  ...
  for (const item of output) {
    ...
    if (itemType === 'function_call') {
      ...
      const toolCall: Record<string, unknown> = {
        id: callId,
        type: 'function',
        function: { name, arguments: normalizeFunctionArguments(itemObj.arguments) || '{}' },
      };
      const extraContent = normalizeToolCallExtraContent(itemObj);
      if (extraContent !== undefined) toolCall.extra_content = extraContent;
  ...
  if (textParts.length === 1 && firstTextPart?.type === 'text') {
    message.content = firstTextPart.text;
  ...
  return {
    ...
    choices: [{ message, finish_reason: detectResponsesFinishReason(responseObj) }],
    usage: {
...
function cacheToolCallExtraContentFromResponsesResponse(body: unknown): void {
  ...
  for (const item of toArray(responseObj.output)) {
    const itemObj = toOptionalObject(item);
    if (!itemObj || toString(itemObj.type) !== 'function_call') continue;
...
function ensureToolUseBlock(
  ...
  if (state.currentBlockType === 'tool_use' && state.activeToolIndex === index) return;
  ...
  if (toolCall.extraContent !== undefined) contentBlock.extra_content = toolCall.extraContent;
...
function processOpenAIChunk(
  ...
  if (Array.isArray(delta?.tool_calls)) {
    for (const item of delta.tool_calls) {
      ...
      if (normalizedExtraContent !== undefined) existing.extraContent = normalizedExtraContent;
      ...
      if (item.function?.name) ensureToolUseBlock(res, state, toolIndex, existing);
...
function findSSEPacketBoundary(buffer: string): { index: number; separatorLength: number } | null {
  ...
  return { index: match.index, separatorLength: match[0].length };
}
...
function extractResponsesFunctionCallMetadata(
  ...
  return { outputIndex, callId, itemId, name, extraContent };
}
...
function registerResponsesFunctionCallState(
  ...
  if (!callState && metadata.itemId) callState = context.functionCallByItemId.get(metadata.itemId);
  ...
  if (metadata.name) callState.name = metadata.name;
  if (metadata.extraContent !== undefined) callState.extraContent = metadata.extraContent;
...
function syncToolCallStateWithResponsesFunctionCall(
  ...
  if (callState.name) toolCall.name = callState.name;
  if (callState.extraContent !== undefined) toolCall.extraContent = callState.extraContent;
...
function emitResponsesFunctionCallChunk(
  ...
  if (options.includeName && toolCall.name) functionObj.name = toolCall.name;

  const argumentsText = options.argumentsText ?? '';
  if (argumentsText) functionObj.arguments = argumentsText;

  if (Object.keys(functionObj).length === 0) return;
...
function emitResponsesFunctionCallMetadataOnce(
  ...
  if (callState.metadataEmitted || !callState.name) return;
  emitResponsesFunctionCallChunk(res, state, callState, { includeName: true, responseId, model });
...
function emitResponsesCompletedFunctionCalls(
  ...
  for (const [index, item] of toArray(responseObj.output).entries()) {
    ...
    payloadObj.output_index = itemOutputIndex !== null ? itemOutputIndex : index;
    ...
    emitResponsesFunctionCallArgumentsOnce(
      res, state, context, callState, finalizedArguments, responseId, model
    );
  }
}
...
function processResponsesStreamEvent(
  ...
  if (eventType === 'response.created') return;
  ...
  if (
    ...
    if (toString(itemObj.type) === 'function_call') {
      ...
      if (eventType === 'response.output_item.done' && !callState.emitted) {
        ...
        if (inlineArguments) {
          emitResponsesFunctionCallArgumentsOnce(
            res, state, context, callState, inlineArguments, responseId, model
          );
        }
      }
    }
  ...
  if (eventType === 'response.function_call_arguments.done') {
    const callState = registerResponsesFunctionCallState(context, payloadObj, null);
    const argumentsDone =
      normalizeFunctionArguments(payloadObj.arguments) ||
      callState.argumentsBuffer ||
      '{}';
    callState.finalArguments = argumentsDone;
    emitResponsesFunctionCallArgumentsOnce(
      res, state, context, callState, argumentsDone,
...
export class OpenAIProxyManager {
  private server: http.Server | null = null;
  private port: number | null = null;
  private lastProxyError: string | null = null;
  private readonly routes = new Map<string, OpenAIProxyRoute>();
  ...
  async start(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const configuredPort = Number(process.env.OPENAI_PROXY_PORT || 0);
      server.listen(configuredPort, PROXY_BIND_HOST, () => {
        ...
        log.info(TAG, "OpenAI compatibility proxy started", { port: this.port });
        resolve();
      });
    });
  }
  ...
  acquire(upstream: ResolvedAgentProfile): ProxyRouteHandle {
    ...
    if (!this.server || !this.port) {
      throw new Error("OpenAI compatibility proxy has not been started.");
    }
  ...
  private resolveRoute(pathname: string): OpenAIProxyRoute | null {
  ...
  private matchesRouteAuth(req: http.IncomingMessage, route: OpenAIProxyRoute): boolean {
  ...
  private async handleRequest(
    ...
    if (method !== "POST" || routePath !== "/v1/messages") {
      writeJSON(res, 404, createAnthropicErrorBody("Not found", "not_found_error"));
      return;
    }
    ...
    const upstreamAPIType = resolveUpstreamAPIType(upstream);
    const openAIRequest = anthropicToOpenAI(parsedRequestBody);
    if (!openAIRequest.model) {
      openAIRequest.model = upstream.model;
    }
    ...
    if (
      ...
      if (requestModel !== upstream.model) {
        openAIRequest.model = upstream.model;
      }
    }

    filterOpenAIToolsForProvider(openAIRequest, upstream.provider);
    remapMessageRolesForMiniMax(openAIRequest, upstream.provider);
