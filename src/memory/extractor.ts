...
const EXPLICIT_ADD_PREFIX_RE = /^(?:请)?(?:记住|记下|保存到记忆|保存记忆|写入记忆|remember(?:\s+this|\s+that)?|store\s+(?:this|that)\s+in\s+memory)\s*[:：,，]?\s*/i;
const EXPLICIT_DELETE_PREFIX_RE = /^(?:请)?(?:删除记忆|从记忆中删除|忘掉|忘记这条|forget\s+this|remove\s+from\s+memory)\s*[:：,，]?\s*/i;
...
function sanitizeImplicitCandidate(text: string): string {
  ...
  const clipped = tailMatch?.index && tailMatch.index > 0
    ...
    : normalized;
  if (EXPLICIT_DELETE_PREFIX_RE.test(clipped)) {
    return "";
  }
  return normalizeText(
    clipped
      .replace(EXPLICIT_ADD_PREFIX_RE, "")
      .replace(/[，,；;:\-]+$/, ""),
  );
}

function confidenceThreshold(level: MemoryGuardLevel): number {
